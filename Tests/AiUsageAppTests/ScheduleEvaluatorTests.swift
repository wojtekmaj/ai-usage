...
struct ScheduleEvaluatorTests {
    ...
    @Test
    func behindAlertDoesNotRetriggerWhenOnlyResetDateWobbles() {
        let evaluator = ScheduleEvaluator()
        ...
        let wobblingMetric = UsageMetric(
            kind: .codexWeekly,
            remainingFraction: 0.79,
            remainingValue: 79,
            totalValue: 100,
            unit: .percentage,
            resetAtUTC: firstResetAt.addingTimeInterval(1),
            lastUpdatedAtUTC: now.addingTimeInterval(5 * 60),
            detailText: nil
        )

        let second = evaluator.evaluate(
            metric: wobblingMetric,
            direction: .behind,
